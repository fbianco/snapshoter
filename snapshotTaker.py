...
"""
	...
	Snapshoter allow to take full screen snapshot each give number of sec and storing it to an image file.
	
	
	\section Infos
	
	Written by fbianco aka François Bianco, University of Geneva - francois.bianco@unige.ch
	
	
	\section Copyright

	Copyright © 2009 fbianco aka François Bianco, University of Geneva - francois.bianco@unige.ch
...
class SnapshoterWindow(Qt.QMainWindow) :
	""" Snapshoter construct a widget with all configuration options and define the main function of this application. """
	
	def __init__(self, application = None, *args) :
		""" Constructor : Application argument is used for connecting the quitAction to the quit slot of the application."""
		...
		self.startButton = Qt.QPushButton( Qt.QIcon( 'img/start.svg' ), _tr('Start') )
		self.startButton.setCheckable( True )
		Qt.QObject.connect( self.startButton, Qt.SIGNAL("clicked()"), self.toggleSnapshoter )
		...
		self.changeSnapshotNameButton = Qt.QPushButton( Qt.QIcon( 'img/saveAs.svg'), _tr('Select file') )
		Qt.QObject.connect( self.changeSnapshotNameButton, Qt.SIGNAL("clicked()"), self.selectFile )
	...
	def selectFile( self ):
		...
		fileName = Qt.QFileDialog.getSaveFileName(self, _tr("Snapshot file"),self.snapshotName.text(),
	...
	def createActions( self ):
		...
		Qt.QObject.connect(self.toggleVisibilityAction, Qt.SIGNAL("triggered()"), self.toggleVisibility)
		
		
	def getScreenGeometry( self ):
		""" Construct a rectangle containing all the screens active on the desktop. """
	...
	def createTrayIcon(self):
		...
		Qt.QObject.connect(self.trayIcon,
			Qt.SIGNAL("activated(QSystemTrayIcon::ActivationReason)"), self.toggleVisibility )
		
		
	def toggleVisibility(self, reason = None):
		""" Show or hide the window, except if the activation reason (form the tray icon) required the context menu. """
	...
	def closeEvent(self, event):
		""" Reimplement the close event (like click on the 'x' on the window decoration)."""
	...
	def toggleSnapshoter(self):
		...
		if self.startButton.isChecked() :
			...
			if DEBUG: print "Started each %i seconds."% self.intervalSpinBox.value()
		else :
	...
	def timerEvent(self, event = None):
		...
		if DEBUG: print "Try to save snapshot to %s. Result %s" % (self.snapshotName.text(),ok)
		
		if not ok :
			self.trayIcon.showMessage ( _tr('Error'), _tr('The snapshot cannot be saved under %s.\nSnapshoter will be stopped.') % self.snapshotName.text(), Qt.QSystemTrayIcon.Critical)
